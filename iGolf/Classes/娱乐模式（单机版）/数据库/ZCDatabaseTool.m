...
+(BOOL)ToCreateAGame:(NSDictionary *)dict
{
   ...
   BOOL success2=[_db executeUpdate:@"INSERT INTO t_player(is_owner,nickname,portrait,match_id) VALUES (?,?,?,?);",@(player.isUser),player.name,userImageName,@(lastId)];
    NSInteger lastUserId = [_db lastInsertRowId];
    tool.userID=lastUserId;
    ZCLog(@"%ld",(long)lastUserId);
   BOOL success3=[_db executeUpdate:@"INSERT INTO t_player(is_owner,nickname,portrait,match_id) VALUES (?,?,?,?);",@(player.isUser),player1.name,otherImageName,@(lastId)];
